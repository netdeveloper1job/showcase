...
declare var $:any;
@Component({
  selector: 'app-addauditrequest-popup',
  templateUrl: './addauditrequest-popup.component.html',
  styleUrls: ['./addauditrequest-popup.component.scss']
})
export class AddauditrequestPopupComponent implements OnInit {
	selected='';
	itemsparking:any;
	itemsorg:any[];
	selectedorg:any;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddauditrequestPopupComponent>,
  ...
  ngOnInit() {
	if(sessionStorage.getItem('typeofloginUser')=='Admin'){
		this.dataservice.allparkingswithoutstatus()
		  .subscribe(data => {
  ...
  // searchques(query: string){
    // let result = this.selectques(query.toLowerCase())
    // this.selectedques = result;
  // }
  
  // selectques(query: string):string[]{
    // let result: string[] = [];
