...
@Component({
  selector: 'app-popupbankcardmaster',
  templateUrl: './popupbankcardmaster.component.html',
  styleUrls: ['./popupbankcardmaster.component.scss']
})
export class PopupbankcardmasterComponent implements OnInit {
		public itemForm: FormGroup;
		public file: any;
		public url = 'http://placehold.it/180';	
  constructor(
		@Inject(MAT_DIALOG_DATA) public data: any,
		public dialogRef: MatDialogRef<PopupbankcardmasterComponent>,
  ...
  buildItemForm(item) {
		...
		if(item.BankCardLogo){
		this.url = this.imagepathurl+item.BankCardLogo;
		}else{
			this.url = this.imagepathurl+'public/ServiceLogo/service.png';
		}
		this.itemForm = new FormGroup({
		  BankCardName: new FormControl(item.BankCardName || '', Validators.required),
	...
	let formData = new FormData();
		...
		formData.append('BankCardName', this.itemForm.get('BankCardName').value);
