...
@Component({
  selector: 'app-popupservice-level',
  templateUrl: './popupservice-level.component.html',
  styleUrls: ['./popupservice-level.component.scss']
})
export class PopupserviceLevelComponent implements OnInit {
	public itemForm: FormGroup;
	public file: any;
	public url = 'http://placehold.it/180';	
  constructor(
	@Inject(MAT_DIALOG_DATA) public data: any,
	public dialogRef: MatDialogRef<PopupserviceLevelComponent>,
	...
	buildItemForm(item) {
			...
			if(item.ServiceLevelLogo){
			this.url = this.imagepathurl+item.ServiceLevelLogo;
			}else{
				this.url = this.imagepathurl+'public/ServiceLogo/service.png';
			}
			this.itemForm = new FormGroup({
			  ServiceLevelName: new FormControl(item.ServiceLevelName || '', Validators.required),
	...
	submit() {
			...
			let formData = new FormData();
				...
				formData.append('ServiceLevelName', this.itemForm.get('ServiceLevelName').value);
